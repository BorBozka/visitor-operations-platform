...
export async function registerVisitorOperationsRoutes(app: FastifyInstance, dependencies: { service: VisitorOperationsService; guards: AuthGuards }) {
  ...
  app.patch("/api/security/visits/:id/correction", { preHandler: securityGuard }, async (request) => service.correctVisitor(parsed(idParams.safeParse(request.params)).id, parsed(z.object({ firstName: z.string().max(100), lastName: z.string().max(100), email: z.string().max(320).optional(), company: z.string().max(200), phone: z.string().max(40).optional(), hostEmployeeName: z.string().max(200) }).strict().safeParse(request.body)), toAccessContext(request.currentUser!)))
