...
import type { MeetingDto, VisitDto, VisitStatus } from "./types.js"

/**
 * The unauthenticated pre-registration endpoints, driven through the real HTTP stack.
 *
 * A visit that leaves `PLANNED` between the service's pre-check and the repository's write
...
/** Mirrors the production repository: the persisted status is re-checked at write time. */
function publicRepository(state: { status: VisitStatus; statusAtWriteTime?: VisitStatus }) {
  ...
  const revalidate = () => { if (writeStatus() !== "PLANNED") throw new PublicInvitationInactiveError() }
  const visit = (): VisitDto => ({ id: "visit-1", meetingId: meeting.id, visitor, status: state.status, invitationStatus: "SENT", createdAt: at, updatedAt: at, meeting })
  return {
...
describe("public invitation routes", () => {
  const state: { status: VisitStatus; statusAtWriteTime?: VisitStatus } = { status: "PLANNED" }
  ...
  it("serves and mutates a PLANNED invitation", async () => {
    state.status = "PLANNED"; state.statusAtWriteTime = undefined
  ...
  it.each(["CANCELLED", "CHECKED_IN"] as const)("hides a visit that turned %s at write time behind the unknown-token 404", async (status) => {
    ...
    state.status = "PLANNED"; state.statusAtWriteTime = status
